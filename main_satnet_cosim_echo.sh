...
if [ "$#" -ne 1 ]; then
    echo
    echo "Usage: main_satnet_cosim_echo <hypatia-root-dir>"
    echo
...
cp -R ${DEMO_DIR}/src/main_satnet_cosim_echo/ ${HYPATIA_ROOT_DIR}/ns3-sat-sim/simulator/scratch

# Run the simulation
cd ${HYPATIA_ROOT_DIR}/ns3-sat-sim/simulator; 
./waf --run="main_satnet_cosim_echo --run_dir='${RUN_DIR}'" 2>&1 | tee ${LOG_DIR}/console.txt
