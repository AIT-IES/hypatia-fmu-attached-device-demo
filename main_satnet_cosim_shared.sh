...
if [ "$#" -ne 1 ]; then
    echo
    echo "Usage: main_satnet_cosim_shared <hypatia-root-dir>"
    echo
...
RUN_DIR=${DEMO_DIR}/demo_satnet_shared_run_dir # 
...
cp -R ${DEMO_DIR}/src/main_satnet_cosim_shared/ ${HYPATIA_ROOT_DIR}/ns3-sat-sim/simulator/scratch

# Run the simulation
cd ${HYPATIA_ROOT_DIR}/ns3-sat-sim/simulator; 
./waf --run="main_satnet_cosim_shared --run_dir='${RUN_DIR}'" 2>&1 | tee ${LOG_DIR}/console.txt
